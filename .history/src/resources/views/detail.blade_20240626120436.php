            ...
            <div class="item-detail__content">
                ...
                @if (Auth::check())
                    @if(count($item->likes) == 0)
                        <form class="favorite-item" method="POST" action="{{ route('/item/like',['item_id' => $item->id]) }}">
                            @csrf
                            <input class="like-icon inactive" type="image" src="/images/unlike.png" alt="いいね" width="32px" height="32px" />
                        </form>
                        @else
                        <form class="favorite-item"form action="favorite-item" method="POST" action="/item/unlike/{item_id}">
                            @csrf
